...
现在，可以在任何 `.vue`文件的 `<template>`中直接使用图标，无需 `import`。

好的，这是一份详细的笔记，汇整了在 Vite + Vue 3 项目中配置和使用 unplugin-icons 的完整流程和配置详解。

笔记：Vite + Vue 3 终极图标解决方案 (unplugin-icons)
这篇笔记的目标是带你配置一套“一次配置，永久享受”的图标工作流。

最终效果：

无需 import：在模板中直接使用 <i-mdi-home /> 即可渲染图标。

按需加载：只有用到的图标才会被打包，极致优化性能。

本地支持：可将你自己的 src/icons 目录下的 SVG 当作图标组件使用 (如 <i-local-my-logo />)。

IDE 友好：VSCode (或其它 IDE) 会自动补全图标名，且 TypeScript 不会报错。

样式可控：可以用 font-size 和 color 像控制文字一样控制图标的大小和颜色。

我们将使用 unplugin-icons 和 unplugin-vue-components 这对“黄金搭档”来实现。
...
# -D 表示安装为开发依赖 (devDependencies)
npm install unplugin-icons unplugin-vue-components -D
unplugin-icons: 核心插件，负责识别、查找和编译图标。

unplugin-vue-components: 辅助插件，负责自动导入 Vue 组件。是它让我们免去了手写 import 的步骤。

注意： 你不需要手动安装任何图标集 (如 @iconify/json-mdi)。unplugin-icons 默认启用了 autoInstall: true，它会在你首次使用某个图标集 (如 mdi) 时自动帮你安装。
...
步骤三：配置 TypeScript (tsconfig.app.json)
目的： 让 TypeScript 识别自动生成的 components.d.ts 文件，消除 IDE 报错并提供类型提示。

重要提示： 不要修改根目录的 tsconfig.json！Vite 项目使用 references 来组织 TS 配置，App 相关的配置在 tsconfig.app.json 中。
...
操作完成后，必须重启 VSCode (或执行 Developer: Reload Window 命令)！
